    ...
    for (var diag = -size; diag <= size; diag += 2) {
      ...
      if (x >= lenA && y >= lenB) {
        ...
        for (var i$1 = size - 1; i$1 >= 0; i$1--) {
          var next$1 = frontier[diag + 1 + max],
            prev$1 = frontier[diag - 1 + max];
          if (next$1 < prev$1) {
            // Deletion
            diag--;
            x = prev$1 + start;
...
prototypeAccessors.lenA.get = function () {
  return this.toA - this.fromA
};
prototypeAccessors.lenB.get = function () {
...
prototypeAccessors$1.startDoc.get = function () {
